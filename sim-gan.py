...
def discriminator_network(input_image_tensor):
    ...
    """
    x = layers.Convolution2D(96, 3, 3, padding='same', subsample=(2, 2), activation='relu')(input_image_tensor)
    x = layers.Convolution2D(64, 3, 3, padding='same', subsample=(2, 2), activation='relu')(x)
    x = layers.MaxPooling2D(pool_size=(3, 3), padding='same', strides=(1, 1))(x)
    x = layers.Convolution2D(32, 3, 3, padding='same', subsample=(1, 1), activation='relu')(x)
    x = layers.Convolution2D(32, 1, 1, padding='same', subsample=(1, 1), activation='relu')(x)
    x = layers.Convolution2D(2, 1, 1, padding='same', subsample=(1, 1), activation='relu')(x)
